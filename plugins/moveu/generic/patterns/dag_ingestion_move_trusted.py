...
class DagIngestionMoveTrusted(BaseDagPatterns):
    ...
    def define_cluster(self, dag: DAG, region: str = "US") -> None:
        self.cluster = self._define_cluster(
            ...
            region=region,
            suffix_cluster_name="move-trd-{{ dag_run.conf['ID']}}".lower(),
    ...
    # -------------------------------------------------------------
    def move_raw_to_trusted(self, dag: DAG, project_lake: str):

        return self._submit_job(
            dag=dag,
            task_id=f"move_raw_to_trusted",
