...
from libs.generic.patterns.dag_ingestion_table_patterns import (
    DagIngestionTablePatterns as DagPatterns,
)
...
INFO_RAW_INGESTION = DagPatterns.get_custom_variable(
    "INFO_DEFAULT_RAW_INGESTION_TABLES", BU
)
...
JDBC_CONNECTIONS = DagPatterns.get_custom_variable(
    INFO_RAW_INGESTION["JDBC_CONNECTIONS"]
)
...
dag_patterns = DagPatterns(
    ...
    dag_connections=JDBC_CONNECTIONS,
...
for table in dag_patterns.cfg_ingestion:
    cluster = dag_patterns.get_cluster_for_table(table.RAW_TABLE)

    ingestion_raw = dag_patterns.raw_update_table(
        dag=dag,
        task_group=cluster.CLUSTER_GROUP,
        cluster=cluster,
        destination_table=table.RAW_TABLE,
        cfg_table=table,
        secret_id_jdbc_credentials=JDBC_CONNECTIONS["ID_SECRET"],
    ...
    load_data_to_bigquery = dag_patterns.load_data_to_bigquery(
        ...
        expiration_ms=table.EXPIRATION_MS,
    ...
    (
        initial_vars
        >> define_date_setup
        >> dag_patterns.branching
        >> cluster.CLUSTER_CREATED
        >> ingestion_raw
        >> ingestion_trd
        >> load_data_to_bigquery
        >> data_quality
        >> cluster.CLUSTER_DELETE
        >> complete_process
    )
