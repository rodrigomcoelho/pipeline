...
from libs.generic.patterns.dag_ingestion_gdr_patterns import (
    DagIngestionFilePatterns as DagPatterns,
)
...
for index, file in enumerate(dag_patterns.cfg_ingestion):
    checking_files = dag_patterns.checking_files(index=index, file=file, dag=dag)

    copy_gdrive_to_gcs = dag_patterns.copy_gdrive_to_gcs(
        index=index, file=file, dag=dag
    )
