...
from libs.generic.patterns.dag_ingestion_ftp_patterns import (
    DagIngestionFtpPatterns as DagPatterns,
)
...
FTP_CREDENTIALS = DagPatterns.get_custom_variable(INFO_RAW_INGESTION["FTP_CREDENTIALS"])
...
for file in dag_patterns.cfg_ingestion:

    convert = dag_patterns.convert_to_utf8(
        dag,
        process_date=PROCESS_DATE,
        task_id=f"convert_to_utf8_{file.RAW_TABLE}",
        file=file,
        sftp_conn=FTP_CREDENTIALS,
        destination_bucket=INFO_RAW_INGESTION["ORIGINAL_BUCKET"],
    )
    ...
    load_data_to_bigquery = dag_patterns.load_data_to_bigquery(
        ...
        type_partition=file.TYPE_PARTITION,
